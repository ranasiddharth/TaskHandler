...
const Form = ({ handleUpdateClose }) => {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    await axios
      .put(
        `http://127.0.0.1:8000/gotasks/projects/${proj_id}/`,
        formData,
        config
      )
      .then((res) => {
  ...
  return (
    <form className={classes.root} onSubmit={handleUpdateSubmit}>
      <h3 style={{ color: "red" }}>
        {err
          ? "Updation of project unsuccessful! Available for only admins and project members."
          : ""}
      </h3>
      <h3 style={{ color: "red" }}>
        {duperr ? "Project with this name already exists" : ""}
      </h3>
      <TextField
        label="Name"
        variant="filled"
